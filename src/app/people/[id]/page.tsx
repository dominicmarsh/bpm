...
export default async function PersonPage({ params }: { params: { id: string } }) {
  ...
  const meetings = person.meetings.map((mp) => mp.meeting).filter((m) => m.biometrics)
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        <div>
          ...
          <h1 className="text-2xl font-bold text-text-primary mt-2">{person.name ?? person.email}</h1>
          <p className="text-text-secondary text-sm">{person.email}</p>
          {person.team && <p className="text-text-secondary text-xs mt-1">Team: {person.team}</p>}
        </div>
